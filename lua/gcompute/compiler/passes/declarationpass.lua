...
function self:ProcessBlock (compilationUnit, block)	
	for statement in block:GetEnumerator () do
		...
		elseif statement:Is ("VariableDeclaration") then
			...
			compilationUnit:Error ("DeclarationPass: Unhandled AST node " .. statement:GetNodeType () .. " (" .. statement:ToString () .. ")", statement:GetSourceLine (), statement:GetSourceCharacter ())
		end
	end
end
