...
function self:VisitExpression (expression)
	...
	elseif expression:Is ("BooleanLiteral") then
		expression:SetType (GCompute.DeferredObjectResolution ("bool", GCompute.ResolutionObjectType.Type, self.GlobalNamespace):Resolve ())
	elseif expression:Is ("NumericLiteral") then
		expression:SetType (GCompute.DeferredObjectResolution ("Expression2.number", GCompute.ResolutionObjectType.Type, self.GlobalNamespace):Resolve ())
	elseif expression:Is ("StringLiteral") then
		expression:SetType (GCompute.DeferredObjectResolution ("Expression2.string", GCompute.ResolutionObjectType.Type, self.GlobalNamespace):Resolve ())
	end
end