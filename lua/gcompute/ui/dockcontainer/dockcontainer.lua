...
function PANEL:DrawDropOverlay (renderContext)
	...
	if not self:IsRootDockContainer () or self.DockContainerType ~= GCompute.DockContainer.DockContainerType.SplitContainer then
		renderContext:PushViewPort (self:LocalToScreen (0, 0))
...
function PANEL:DrawDropTargets (renderContext, alphaScale, dropData)
	...
	if self:GetContainerType () == GCompute.DockContainer.DockContainerType.None then return end
...
function PANEL:OnDragOver (dragDropController, x, y)
	...
	if self:GetRootDockContainer ():GetActiveRootDropButton () ~= GCompute.DockContainer.DropTarget.None then
		self:SetActiveDropButton (GCompute.DockContainer.DropTarget.None)
		self:SetActiveDropTarget (GCompute.DockContainer.DropTarget.None)
		return
	end
	...
	if not self:IsRootDockContainer () or
	   self:GetContainerType () ~= GCompute.DockContainer.DockContainerType.SplitContainer then
		...
		if self:GetContainerType () == GCompute.DockContainer.DockContainerType.None then
		...
		if self:GetContainerType () == GCompute.DockContainer.DockContainerType.TabControl and
