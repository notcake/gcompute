...
function PANEL:Init ()
	...
	-- Caret
	self.CaretLocation = GCompute.IDE.LineColumnLocation ()
	self.PreferredCaretLocation = GCompute.IDE.LineColumnLocation ()
	self.CaretBlinkTime = SysTime ()
	
	-- Selection
	self.Selecting = false
	self.Selection = GCompute.IDE.TextSelection (self)
	self.SelectionController = GCompute.IDE.TextSelectionController (self, self.Selection)
	...
	self.BracketHighlighter = GCompute.IDE.BracketHighlighter (self)
	
	-- Settings
	self.TextRenderer = GCompute.IDE.TextRenderer ()
	...
	self.ViewLocation = GCompute.IDE.LineColumnLocation ()
	...
	-- Autocomplete
	self.CodeCompletionProvider = GCompute.IDE.CodeCompletion.CodeCompletionProvider (self)
	...
	self:SetKeyboardMap (GCompute.IDE.CodeEditorKeyboardMap)
...
function PANEL:DrawCaret ()
	...
	if self:GetSelectionMode () == GCompute.IDE.SelectionMode.Regular then
		self:DrawCaretRegular ()
	elseif self:GetSelectionMode () == GCompute.IDE.SelectionMode.Block then
		self:DrawCaretBlock ()
	end
end
...
function PANEL:Clear ()
	self.Document:Clear ()
	self:SetCaretPos (GCompute.IDE.LineColumnLocation (0, 0))
	self:SetSelection (GCompute.IDE.LineColumnLocation (0, 0), GCompute.IDE.LineColumnLocation (0, 0))
end
...
function PANEL:SetDocument (document)
	...
	if self.Document then
		...
		if not self.Document.SyntaxHighlighter then
			self.Document.SyntaxHighlighter = GCompute.IDE.SyntaxHighlighter (self.Document)
			self:GetSyntaxHighlighter ():SetEnabled (self:IsCompilationEnabled ())
		end
		if not self.Document.IdentifierHighlighter then
			self.Document.IdentifierHighlighter = GCompute.IDE.IdentifierHighlighter (self.Document, self.Document.SyntaxHighlighter)
...
function PANEL:DeleteSelection ()
	...
	local deletionAction = GCompute.IDE.DeletionAction (self, selectionStartLocation, selectionEndLocation, selectionStart, selectionEnd, text)
	deletionAction:Redo ()
...
function PANEL:IndentSelection ()
	local indentationAction = GCompute.IDE.IndentationAction (self, self:CreateSelectionSnapshot ())
	indentationAction:Redo ()
	self:GetUndoRedoStack ():Push (indentationAction)
end

function PANEL:OutdentSelection ()
	local outdentationAction = GCompute.IDE.OutdentationAction (self, self:CreateSelectionSnapshot ())
...
function PANEL:ReplaceSelectionText (text, pasted)
	...
	if self.Selection:GetSelectionMode () == GCompute.IDE.SelectionMode.Block and
	   self.Selection:IsMultiline () then
		undoRedoItem = GCompute.IDE.BlockReplacementAction (self, self:CreateSelectionSnapshot (), text)
	elseif self.Selection:IsEmpty () then
		...
		undoRedoItem = GCompute.IDE.InsertionAction (self, insertionLocation, text)
	else
		local selectionStart = self.Document:ColumnToCharacter (self.Selection:GetSelectionStart (), self.TextRenderer)
		local selectionEnd   = self.Document:ColumnToCharacter (self.Selection:GetSelectionEnd (),   self.TextRenderer)
		local originalText = self.Document:GetText (selectionStart, selectionEnd)
		
		undoRedoItem = GCompute.IDE.ReplacementAction (self, selectionStart, selectionEnd, originalText, text)
	end
	undoRedoItem:Redo ()
	...
	if not pasted then
		...
		if self:GetEditorHelper ():ShouldOutdent (self, self.Document:ColumnToCharacter (self:GetCaretPos (), self.TextRenderer)) then
			autoOutdentationAction = autoOutdentationAction or GCompute.IDE.AutoOutdentationAction (self)
		...
		if autoOutdentationAction then
			...
			-- Update caret
			if self.Selection:GetSelectionMode () == GCompute.IDE.SelectionMode.Regular then
...
function PANEL:ReplaceText (startLocation, endLocation, text)
	...
	local undoRedoItem = GCompute.IDE.ReplacementAction (self, startLocation, endLocation, originalText, text)
	undoRedoItem:Redo ()
	self:GetUndoRedoStack ():Push (undoRedoItem)
end
...
function PANEL:MoveCaretLeft (toWordBoundary, overrideSelectionStart)
	if self.CaretLocation:GetColumn () == 0 then
		...
		if toWordBoundary then
			local newLocation = self.Document:CharacterToColumn (self.Document:GetPreviousWordBoundary (GCompute.IDE.LineCharacterLocation (lineNumber, character)), self.TextRenderer)
...
function PANEL:MoveCaretRight (toWordBoundary, overrideSelectionStart)
	if self.CaretLocation:GetColumn () == self.Document:GetLine (self.CaretLocation:GetLine ()):GetColumnCount (self.TextRenderer) then
		...
		self.SelectionController:SelectLocation (GCompute.IDE.LineColumnLocation (
			self.CaretLocation:GetLine () + 1,
			0
		...
		if toWordBoundary then
			local newLocation = self.Document:CharacterToColumn (self.Document:GetNextWordBoundary (GCompute.IDE.LineCharacterLocation (lineNumber, character)), self.TextRenderer)
			lineNumber = newLocation:GetLine ()
			column     = newLocation:GetColumn ()
		else
...
function PANEL:MoveCaretUp (overrideSelectionStart)
	if self.CaretLocation:GetLine () == 0 then
		self:SetRawCaretPos (GCompute.IDE.LineColumnLocation (0, 0))
	else
		self:SetPreferredCaretPos (GCompute.IDE.LineColumnLocation (
...
function PANEL:SetCaretPos (caretLocation, scrollToCaret)
	...
	caretLocation = GCompute.IDE.LineColumnLocation (line, self:FixupColumn (line, caretLocation:GetColumn ()))
...
function PANEL:SetPreferredCaretPos (preferredCaretLocation, scrollToCaret)
	...
	preferredCaretLocation = GCompute.IDE.LineColumnLocation (preferredCaretLocation:GetLine (), self:FixupColumn (preferredCaretLocation:GetLine (), preferredCaretLocation:GetColumn ()))
...
function PANEL:CreateSelectionSnapshot (selectionSnapshot)
	selectionSnapshot = selectionSnapshot or GCompute.IDE.SelectionSnapshot ()
...
function PANEL:SelectAll ()
	self:SetSelectionMode (GCompute.IDE.SelectionMode.Regular)
	self:SetSelection (
...
function PANEL:PointToLocation (x, y)
	...
	elseif column > lineWidth then
		if self:GetSelectionMode () == GCompute.IDE.SelectionMode.Block then
	...
	return GCompute.IDE.LineColumnLocation (line, column)
end
...
function PANEL:CutSelection ()
	...
	if self.Selection:GetSelectionMode () == GCompute.IDE.SelectionMode.Regular then
		...
		deletionAction = GCompute.IDE.DeletionAction (self, selectionStart, selectionEnd, selectionStart, selectionEnd, text)
		deletionAction:SetVerb ("cut")
	else
		deletionAction = GCompute.IDE.BlockDeletionAction (self, self:CreateSelectionSnapshot ())
