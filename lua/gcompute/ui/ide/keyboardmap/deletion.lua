GCompute.IDE.CodeEditorKeyboardMap:Register ({ KEY_BACKSPACE, KEY_DELETE },
	function (self, key, ctrl, shift, alt)
		...
		if self:GetSelection ():GetSelectionMode () ~= GCompute.IDE.SelectionMode.Regular then return false end
		...
		if key == KEY_BACKSPACE then
			...
			elseif self.CaretLocation:GetColumn () == 0 then
				...
				deletionStart = GCompute.IDE.LineCharacterLocation ()
				...
				deletionEnd = GCompute.IDE.LineCharacterLocation (deletionStart)
		...
		local deletionAction = GCompute.IDE.DeletionAction (self, selectionStartLocation, selectionEndLocation, deletionStart, deletionEnd, text)
		deletionAction:Redo ()
