local self = {}
GCompute.IDE.TextSelection = GCompute.MakeConstructor (self)
...
function self:ctor (codeEditor)
	...
	self.SelectionMode  = GCompute.IDE.SelectionMode.Regular
	self.SelectionStart = GCompute.IDE.LineColumnLocation (0, 0)
	self.SelectionEnd   = GCompute.IDE.LineColumnLocation (0, 0)
...
function self:Flatten ()
	if self:GetSelectionMode () == GCompute.IDE.SelectionMode.Block then
		...
		self:SetSelectionStart (
			GCompute.IDE.LineColumnLocation (
...
function self:GetSpanEnumerator (line)
	...
	if self.SelectionMode == GCompute.IDE.SelectionMode.Regular then
...
function self:IsInSelection (location)
	...
	elseif self.SelectionMode == GCompute.IDE.SelectionMode.Block then
		...
		GCompute.Error ("TextSelection:IsInSelection : Not implemented for " .. GCompute.IDE.SelectionMode [self.SelectionMode] .. " selection mode.")
	end
end
