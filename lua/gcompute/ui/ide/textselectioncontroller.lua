local self = {}
GCompute.IDE.TextSelectionController = GCompute.MakeConstructor (self)
...
function self:ctor (codeEditor, textSelection)
	...
	self.InitialSelectionStart = GCompute.IDE.LineColumnLocation ()
	self.InitialSelectionEnd   = GCompute.IDE.LineColumnLocation ()
		...
		function (_, mouseCode, x, y)
			...
			if mouseCode == MOUSE_LEFT then
				...
				if clickedOnLineNumbers then
					...
					self.Selection:SetSelectionMode (GCompute.IDE.SelectionMode.Regular)
					...
					elseif SysTime () - self.LastLeftMouseDownTime < 0.4 then
						...
						if not shift or alt then
							self.Selection:SetSelectionMode (alt and GCompute.IDE.SelectionMode.Block or GCompute.IDE.SelectionMode.Regular)
...
function self:ConvertToRegularSelection (preferredLine)
	if self.Selection:GetSelectionMode () == GCompute.IDE.SelectionMode.Block then
		self.Selection:SetSelectionMode (GCompute.IDE.SelectionMode.Regular)
...
function self:SelectLine (line, modifySelection)
	local spanStart = GCompute.IDE.LineColumnLocation (line, 0)
	local spanEnd
	if self.Editor.Document:GetLine (line + 1) then
		spanEnd = GCompute.IDE.LineColumnLocation (line + 1, 0)
	else
		spanEnd = GCompute.IDE.LineColumnLocation (line, self.Editor.Document:GetLine (line):GetColumnCount (self.Editor:GetTextRenderer ()))
	end
...
function self:SelectWord (lineColumnLocation, modifySelection)
	...
	if lineColumnLocation:GetColumn () > 0 and lineColumnLocation:GetColumn () >= line:GetColumnCount (self.Editor.TextRenderer) then
		location = self.Editor:GetDocument ():ColumnToCharacter (GCompute.IDE.LineColumnLocation (lineColumnLocation:GetLine (), line:GetColumnCount (self.Editor.TextRenderer) - 1), self.Editor:GetTextRenderer ())
	else
	...
	self:SelectSpan (
		GCompute.IDE.LineColumnLocation (location:GetLine (), line:CharacterToColumn (leftCharacter,  self.Editor:GetTextRenderer ())),
		GCompute.IDE.LineColumnLocation (location:GetLine (), line:CharacterToColumn (rightCharacter, self.Editor:GetTextRenderer ())),
		modifySelection
	)
end