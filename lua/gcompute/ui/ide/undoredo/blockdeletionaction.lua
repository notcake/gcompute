local self = {}
GCompute.IDE.BlockDeletionAction = GCompute.MakeConstructor (self, GCompute.UndoRedoItem)
	...
	for lineNumber, startColumn, endColumn in self.SelectionSnapshot:GetSelectionEnumerator () do
		...
		if startCharacter < line:GetLengthExcludingLineBreak () or startColumn == endColumn then
			...
			elseif self.Mode == "previousword" then
				local wordBoundary = self.CodeEditor.Document:GetPreviousWordBoundary (GCompute.IDE.LineCharacterLocation (lineNumber, startCharacter))
			...
			elseif self.Mode == "nextword" then
				local wordBoundary = self.CodeEditor.Document:GetNextWordBoundary (GCompute.IDE.LineCharacterLocation (lineNumber, startCharacter))
				if wordBoundary:GetLine () ~= lineNumber then
